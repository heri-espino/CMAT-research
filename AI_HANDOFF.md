...
Before proposing or implementing any Python function, search **`code/FUNCTION_INDEX.md`**. It is the canonical token-efficient inventory of the active code tree. Search by capability, likely function name, and tags; then open only the relevant implementation. Reuse/extend existing code rather than creating parallel helpers when the intended scientific operation already exists.
...
`code/FUNCTION_INDEX.md` is now the active code-discovery layer. Use it before reading broad portions of `src/` or writing new helpers. It contains the full generated symbol inventory and a compact capability map for cohorts/revalidations, outcomes/imputation, PPA persistence, statistics, selection, temporal analyses, programme/visit-count extensions, plots/reporting, privacy/release and entry points.
...
1. read this file;
2. read `AGENTS.md`;
3. **before any code work, search `code/FUNCTION_INDEX.md`;**
4. read `code/README.md` if code/results are relevant;
5. read `docs/PUBLICATION_PORTFOLIO.md` if manuscript strategy is relevant;
6. read `docs/MIGRATION_STATUS.md` and current methodology docs if code/results are relevant;
7. read `literature/AI_HANDOFF.md` if literature is relevant;
8. inspect `main` and any current PR before writing or modifying artifacts.

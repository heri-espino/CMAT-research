...
from cmat_analysis.paths import (
    DEFAULT_ASESORIAS_PATH,
    DEFAULT_MATERIAS_PATH,
    PROJECT_ROOT,
    REPORT_ASSETS_DIR,
)
from cmat_analysis.release_figures import FIGURE_OUTPUTS
...
from cmat_analysis.reporting.render import write_csv_tables, write_json_summary, write_readme, write_tex_snippets


LEGACY_DESCRIPTIVE_FIGURE_NAMES = {
    "visits_histogram.pdf",
    "visits_histogram.png",
    "visits_histogram_low_counts.pdf",
    "visits_histogram_low_counts.png",
    "visits_ecdf.pdf",
    "visits_ecdf.png",
    "visits_tail_curve.pdf",
    "visits_tail_curve.png",
    "visits_continuation_curve.pdf",
    "visits_continuation_curve.png",
    "visits_lorenz_curve.pdf",
    "visits_lorenz_curve.png",
    "visits_by_year.pdf",
    "visits_by_year.png",
    "classroom_size_distribution.pdf",
    "classroom_size_distribution.png",
}
...
def _reset_descriptive_figures(path: Path) -> None:
    path.mkdir(parents=True, exist_ok=True)
    descriptive_names = {
        FIGURE_OUTPUTS[key]
        for key in (
    ...
    for name in descriptive_names | LEGACY_DESCRIPTIVE_FIGURE_NAMES:
        target = path / name
        if target.exists():
            target.unlink()
...
def generate_report_assets(
    *,
    ...
    _reset_descriptive_figures(figures_dir)

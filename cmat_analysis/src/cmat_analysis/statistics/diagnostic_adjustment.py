...
def attach_same_term_diagnostic(
    ...
    *,
    ...
    ----------
    df : pandas.DataFrame
        Focal analytical cohort.
    diagnostics : pandas.DataFrame
        Diagnostic records using the controlled diagnostic schema.
    student_col, year_col, session_col : str
        Focal-cohort key columns.
    diagnostic_student_col, diagnostic_year_col, diagnostic_session_col : str
        Diagnostic-table key columns.
    diagnostic_score_col : str
        Numeric diagnostic score, typically percentage.
    diagnostic_type_col : str
        Column identifying the diagnostic examination type.
    diagnostic_type : str or None, default='DMU'
        Optional diagnostic type to retain before matching.

    Returns
    -------
    pandas.DataFrame
        Copy of ``df`` with ``DIAGNOSTIC_PERCENTAGE``, diagnostic-row audit
        columns, and ``DIAGNOSTIC_MATCHED`` appended.
    """
...
def diagnostic_adjusted_exact_visit_models(
    ...
    *,
    ...
    Both models use the same diagnostic-complete sample. The baseline model
    includes exact visit-group indicators, classroom fixed effects, and career
    indicators. The adjusted model adds a standardized diagnostic score. Cluster-
    robust standard errors are computed by classroom.

    Returns
    -------
    coverage : pandas.DataFrame
        Diagnostic coverage and mean diagnostic score by exact visit group.
    contrasts : pandas.DataFrame
        Visit-group coefficients relative to zero visits from the baseline and
        diagnostic-adjusted models, estimated on the identical complete-case
        sample.
    """
    ...
    base_formula = (
        f"{outcome_col} ~ {group_term} + C({classroom_col}) + C({career_col})"
    )
    adjusted_formula = base_formula + " + DIAGNOSTIC_Z"

    models = {
        "same_sample_without_diagnostic": smf.ols(base_formula, data=d).fit(
            cov_type="cluster", cov_kwds={"groups": d[classroom_col]}
        ),
        "plus_diagnostic": smf.ols(adjusted_formula, data=d).fit(
            cov_type="cluster", cov_kwds={"groups": d[classroom_col]}
        ),
    }

...
from cmat_analysis.pipeline.main_pipeline import run_visitas_pipeline
...
def main() -> int:
    ...
    logger = logging.getLogger("cmat_analysis.runner")
    ...
    if "raw_figures" in artifacts:
        ...
        print(f"Auxiliary figures: {raw_figures['professor_distributions_dir']}")
    if "run_log" in artifacts:
        print(f"Run log: {artifacts['run_log']}")
    return 0

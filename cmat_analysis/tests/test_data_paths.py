...
def test_study_config_uses_controlled_pseudonymized_release(tmp_path: Path) -> None:
    controlled = tmp_path / "data" / "controlled"
    controlled.mkdir(parents=True)
    materias = controlled / "Materias_pseudonymized.csv"
    asesorias = controlled / "Asesorias_pseudonymized.csv"
    materias.touch()
    asesorias.touch()
...
def test_study_config_prefers_raw_over_controlled(tmp_path: Path) -> None:
    raw = tmp_path / "data" / "raw"
    controlled = tmp_path / "data" / "controlled"
    raw.mkdir(parents=True)
    controlled.mkdir(parents=True)
    raw_materias = raw / MATERIAS_NAME
    raw_asesorias = raw / ASESORIAS_NAME
    raw_materias.touch()
    raw_asesorias.touch()
    (controlled / "Materias_pseudonymized.csv").touch()
    (controlled / "Asesorias_pseudonymized.csv").touch()

    config = get_study_config(tmp_path)

    assert config.materias_path == raw_materias
    assert config.asesorias_path == raw_asesorias
...
def test_visit_settings_uses_controlled_pseudonymized_release(tmp_path: Path) -> None:
    controlled = tmp_path / "data" / "controlled"
    controlled.mkdir(parents=True)
    materias = controlled / "Materias_pseudonymized.csv"
    asesorias = controlled / "Asesorias_pseudonymized.csv"

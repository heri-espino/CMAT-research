...
`NORMALIZATION_MANIFEST.json` records source hashes, row counts, mapping coverage and integrity checks. `cmat_normalized.sqlite` is a convenience derivative with foreign keys enabled; the CSV files remain the canonical portable representation.

All row-level files remain controlled pseudonymized research data. Normalization reduces redundancy but does not make the data anonymous.

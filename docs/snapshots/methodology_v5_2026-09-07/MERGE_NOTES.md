# Merge notes — exact methodology snapshot restoration
...
- `reports/methodology_report/provenance/SHA256SUMS.txt`: all listed files pass SHA-256 verification.
...
The `v5` label remains here because this document records a specific historical snapshot. The active report itself is named by purpose under `reports/methodology_report/`.

...
from visitas_analysis.release_figures import figure_stem

from .descriptive_pipeline import YEAR_COL, VISIT_COL


def save_figure_pdf(fig: plt.Figure, output_dir: Path, stem: str) -> list[Path]:
...
def plot_visits_histogram(student_visits: pd.DataFrame, output_dir: Path) -> list[Path]:
    ...
    return save_figure_pdf(fig, output_dir, figure_stem("visits_histogram"))
...
def plot_visits_histogram_low_counts(
    ...
    return save_figure_pdf(fig, output_dir, figure_stem("visits_histogram_low_counts"))
...
def plot_visits_ecdf(student_visits: pd.DataFrame, output_dir: Path) -> list[Path]:
    ...
    ax.set_ylim(0, 1)
    return save_figure_pdf(fig, output_dir, figure_stem("visits_ecdf"))
...
def plot_visits_tail_curve(visit_tail: pd.DataFrame, output_dir: Path) -> list[Path]:
    ...
    ax.set_ylim(0, 1)
    return save_figure_pdf(fig, output_dir, figure_stem("visits_tail_curve"))
...
def plot_visits_continuation_curve(visit_tail: pd.DataFrame, output_dir: Path) -> list[Path]:
    ...
    ax.set_ylim(0, 1)
    return save_figure_pdf(fig, output_dir, figure_stem("visits_continuation_curve"))
...
def plot_visits_by_year(student_year_visits: pd.DataFrame, output_dir: Path) -> list[Path]:
    ...
    return save_figure_pdf(fig, output_dir, figure_stem("visits_by_year"))
...
def plot_classroom_size_distribution(classroom_summary: pd.DataFrame, output_dir: Path) -> list[Path]:
    ...
    return save_figure_pdf(fig, output_dir, figure_stem("classroom_size_distribution"))
...
def plot_visits_lorenz_curve(lorenz: pd.DataFrame, output_dir: Path) -> list[Path]:
    ...
    ax.set_ylim(0, 1)
    ax.legend(frameon=False)
    return save_figure_pdf(fig, output_dir, figure_stem("visits_lorenz_curve"))

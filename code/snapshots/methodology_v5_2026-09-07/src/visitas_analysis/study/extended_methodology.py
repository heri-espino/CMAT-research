...
from .cohort import attach_visits
...
def exact_visit_count_summary(
    ...
    *,
    ...
    Counts above ``max_visits`` are intentionally excluded from this table rather
    than pooled, because the purpose is to show the sparse tail transparently.
    The grouped 0/1/2/3/4+ analysis remains the inferential specification.
    """
...
def exact_visit_count_trend_diagnostics(
    ...
    *,
    ...
    These diagnostics are descriptive and secondary to the multiplicity-aware
    0/1/2/3/4+ contrasts.  The stable-tail specification is pre-declared here as
    1--7 visits because counts above 7 are very sparse in the current data.
    """
...
def welch_anova_exact_groups(
    ...
    *,
    ...
    r"""Welch one-way ANOVA using the summary-statistic formulation.

    Let group j have size n_j, mean \bar{x}_j and unbiased variance s_j^2.
    Define w_j=n_j/s_j^2 and W=\sum w_j. Welch's weighted mean is
    \bar{x}_w=\sum w_j\bar{x}_j/W. The statistic uses the standard Welch
    correction for heteroskedasticity and has an approximate F distribution.
    """
    d = add_exact_visit_group(df, visits_col).dropna(subset=[outcome_col]).copy()
    groups = []
    ...
    # Brown--Forsythe is Levene's test centered at the group median. It tests
    # the homogeneity-of-variance assumption without requiring normality as
    # strongly as the mean-centered Levene version. A small p-value motivates
    # the heteroskedastic Welch/Games--Howell route rather than classical ANOVA.
    ...
    *,
    ...
    for i, g1 in enumerate(EXACT_GROUP_ORDER):
        ...
        for g2 in EXACT_GROUP_ORDER[i + 1:]:
            ...
            half = qcrit * gh_se
            # Welch t is included as a transparent two-group reference, but
            # Games--Howell p/CI are the multiplicity-aware primary pairwise result.
...
def fixed_effect_pairwise_exact_groups(
    ...
    *,
    ...
    """All pairwise adjusted contrasts from one additive FE model.

    Model:
        Z = visit_group + classroom fixed effects + official career fixed effects + error

    Covariance is cluster-robust by classroom. Because all pairwise contrasts
    are extracted from a single model, Holm-adjusted p-values are supplied over
    the 10 visit-group contrasts.
    """
...
def build_all_math_attempts_with_outcomes(data, config) -> pd.DataFrame:
    """All observed eligible math-course attempts in periods with CMAT coverage.

    Unit = student-course-period attempt from the seven-course academic extract.
    This is deliberately not described as all university students. Classroom
    standardization includes professor x subject x period because several
    subjects are pooled in this broad descriptive population.
    """
...
def career_ecological_association(summary: pd.DataFrame) -> pd.DataFrame:
    ...
    This is an ecological diagnostic, not an individual-level relationship.
    Only careers satisfying the table's min-n rule are used.
    """
    rows = []
    for population, g in summary.groupby("population", observed=True):
        ...
        # Weighted least squares gives larger careers more influence; the
        # unweighted correlations are also reported because the unit is career.
...
def calc_progressor_mu_cohort(mu_with_outcomes: pd.DataFrame, longitudinal: pd.DataFrame) -> pd.DataFrame:
    ...
    This reproduces the N=4,211 longitudinal risk set in v4. Z-scores must be
    computed *before* subsetting so each student's MU Z remains referenced to
    the complete classroom, not to the selected progressor subset.
    """

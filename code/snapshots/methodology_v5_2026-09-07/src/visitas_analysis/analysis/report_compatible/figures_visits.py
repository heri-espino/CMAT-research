...
from visitas_analysis.release_figures import figure_path
...
def plot_visit_histograms(asesoria_counts, layout: OutputLayout) -> None:
    ...
    fig.savefig(figure_path(layout.pdf_dir, "visit_histogram_main"), bbox_inches="tight")
    ...
    ax.legend()
    ax.set_ylim(0, 225)
    save_figure(fig, figure_path(layout.pdf_dir, "visit_histogram_low_ylim"))
...
def plot_salon_scatter(ultramerge, layout: OutputLayout) -> None:
    ...
    ax.legend()
    save_figure(fig, figure_path(layout.pdf_dir, "salon_scatter"))
...
def plot_student_scatter(ultramerge_means, layout: OutputLayout) -> None:
    ...
    ax.set_ylabel("Calificaci\u00f3n estandarizada (KDE, Z-score)")
    ax.legend()
    save_figure(fig, figure_path(layout.pdf_dir, "student_scatter"))
...
def plot_mean_z_by_visits(ultramerge_means, layout: OutputLayout) -> None:
    ...
    fig.tight_layout()
    ax.set_xlim(0, 34)
    save_figure(fig, figure_path(layout.pdf_dir, "mean_z_by_visits"))
